...
def generate_pattern(guess,answer):
    # unfortunately order matters so there's not much work we can save with memoisation
    patterns[(guess["name"],answer["name"])] = (compare_mv(guess,answer),
...
def generate_patterns(card,i):
    report(i,card["name"])
    [generate_pattern(card,answer) for answer in allcards]

[generate_patterns(card,i) for i,card in enumerate(allcards)]

outjson = json.dumps(patterns,indent=4)

with open("patterns.json", "w", encoding="utf8") as outfile:
